...
// Reinos vivem no game-server (WebSocket). A API só autentica.
router.post('/create-kingdom', verifyToken, (req, res) => {
    res.status(501).json({
        message: 'Cria o reino via WebSocket no game-server (join_game / create_match).',
        gameServerWs: config.gameServer.publicWsUrl
    });
});

router.post('/join-kingdom', verifyToken, (req, res) => {
    res.status(501).json({
        message: 'Junta-te ao reino via WebSocket no game-server.',
        gameServerWs: config.gameServer.publicWsUrl
    });
});
