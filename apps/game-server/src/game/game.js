...
class Game {
  constructor(options = {}) {
    ...
    this.hadMultipleKingdoms = false;
  ...
  addPlayer(id, name, kingdomName = null, userId = null) {
    const startPosition = this.world.getRandomStartPosition();
    const player = new Player(id, name, startPosition, userId);
  ...
  checkVictoryConditions() {
    ...
    if (this.kingdoms.size >= 2) {
      this.hadMultipleKingdoms = true;
    }

    // Vitória por dominação: havia vários reinos e resta só um
    if (this.hadMultipleKingdoms && this.kingdoms.size === 1) {
  ...
  declareWinner(kingdomId, victoryType) {
    ...
    if (!kingdom || this.winnerDeclared) return;
