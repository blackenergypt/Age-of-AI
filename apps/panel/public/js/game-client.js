class GameClient {
    ...
    connectToServer(assignment = null) {
        try {
            const token = (window.AgeMatchmaking && AgeMatchmaking.getAuthToken())
            ...
            if (assignment?.wsUrl && assignment?.action) {
                this.serverUrl = assignment.wsUrl;
                this.assignedNodeId = assignment.nodeId || null;
                this.pendingJoin = {
                    action: assignment.action,
                    joinPayload: assignment.joinPayload || {}
                };
            } else {
            ...
            console.log('Conectando ao servidor WebSocket:', this.serverUrl, this.assignedNodeId || '');
    ...
    joinGame() {
        ...
        if (this.pendingJoin?.action) {
            this.socket.send(JSON.stringify({
                type: this.pendingJoin.action,
                data: {
                    ...this.pendingJoin.joinPayload,
                    token: this.authToken,
                    playerName: this.playerName,
                    kingdomName: this.pendingJoin.joinPayload.kingdomName || this.kingdomName || null
                }
            }));
            return;
        }
